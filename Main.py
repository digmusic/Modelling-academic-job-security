...
--------------------------------------------------------------------------------
FILE: Main.py
DESC: Main program for the simulation
AUTH: thorsilver
VERS: 1.0 March 2017
...
DIR_PREFIX = 'thesim/'          # subdir of current dir *** testing only ***
...
def main():
    ...
    # ************************************************************
    # Inserted to avoid 'datafile' error --- needs attention
    datafile = open(params['prefix'] + ROI_TEST + '.csv', 'w')
    # ************************************************************

    # WCSS SIMULATIONS
    run_wcss_sims(datafile, params, params['prefix'], seed_rng)

    # SINGLE TEST RUN
    run_once(params, seed_rng)

    # Alife XV simulations, thermo
    root_dir = alife_run(run_alife_thermo, params, seed_rng, False)
    print(root_dir)        # *** inserted to avoid unused variable ***

    # alife XV simulations, memory
    root_dir = alife_run(run_alife_sims, params, seed_rng, False)
    print(root_dir)        # *** inserted to avoid unused variable ***
